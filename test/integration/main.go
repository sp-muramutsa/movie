...
func main() {
	...
	log.Println("Saving metadata via metadata service")
	m := &gen.Metadata{
		Id:          "the-movie",
		Title:       "The Movie",
	...
	log.Println("Retrieving data via metadata service")
	getMetadataResp, err := metadataClient.GetMetadata(ctx, &gen.GetMetadataRequest{MovieId: m.Id})
	if err != nil {
		log.Fatalf("get metadata: %v", err)
	}
	if diff := cmp.Diff(getMetadataResp.Metadata, m, protocmp.Transform()); diff != "" {
		log.Fatalf("get metadata put mismatch: %v", diff)
	}
	...
	if diff := cmp.Diff(getMovieDetailsResp.MovieDetails, wantMovieDetails, protocmp.Transform()); diff != "" {
		log.Fatalf("get movie details put mismatch: %v", diff)
	}
	...
	if got, want := getAggregatedRatingResp.RatingValue, float64(5); got != want {
		log.Fatalf("getAggregatedRatingResp got %v, want %v", got, want)
	}
...
func startMetadataService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	...
	id := discovery.GenerateInstanceID(metadataServiceAddr)
...
func startRatingService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	...
	id := discovery.GenerateInstanceID(ratingServiceAddr)
...
func startMovieService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	log.Println("Starting movie service: " + movieServiceAddr)
	...
	id := discovery.GenerateInstanceID(movieServiceAddr)
